/*
Num dia de sol, você deseja medir a altura de um prédio, porém, a trena não é 
suficientemente longa. Assumindo que seja possível medir sua sombra e a do prédio no 
chão, e que você lembre da sua altura, faça um programa para ler os dados necessários e 
calcular a altura do prédio
*/
#include <stdio.h>
#include <stdlib.h>
int main(){
float alturaPredio, sombraPredio, alturaPessoa, sombraPessoa;
printf("Insira o valor do comprimento da sombra do predio. \n");
scanf("%f", &sombraPredio);
printf("Insira o valor comprimento da sua sombra. \n");
scanf("%f", &sombraPessoa);
printf("Insira o valor do comprimento da sua altura. \n");
scanf("%f", &alturaPessoa);
alturaPredio = alturaPessoa*sombraPredio/sombraPessoa;
printf("Altura do predio: %0.2f.\n", alturaPredio);
system("pause");
return 0;
}